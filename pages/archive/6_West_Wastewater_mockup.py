...
if check_password():
    ...
    @st.cache_data
    def location():
        df = pd.read_csv('data/wastewater_virus_surveillance/west/west_locations.csv')
        df = df.set_index('ID')
        return df
    ...
    def read_distribution():
        df = pd.read_csv('data/wastewater_virus_surveillance/west/west_read_distribution_table.csv')
    ...
    @st.cache_data
    def freyja_by_date():
        df = pd.read_csv('data/wastewater_virus_surveillance/west/freyja/freyja_grouped_avg_by_date_west.csv')
        return df


    @st.cache_data
    def freyja_by_site():
        df = pd.read_csv('data/wastewater_virus_surveillance/west/freyja/freyja_grouped_avg_by_site_west.csv')
    ...
    adeno_current = Image.open('data/wastewater_virus_surveillance/images/current/west/scatter/weekly_counts_Adenovirus_scatter.png')
    cov2_historical = Image.open('data/wastewater_virus_surveillance/images/historical/grouped_counts_SARS-CoV-2_scatter.png')
    cov2_current = Image.open('data/wastewater_virus_surveillance/images/current/west/scatter/weekly_counts_SARS-CoV-2_scatter.png')
    flu_historial = Image.open('data/wastewater_virus_surveillance/images/historical/grouped_counts_Influenza_scatter.png')
    flu_current = Image.open('data/wastewater_virus_surveillance/images/current/west/scatter/weekly_counts_Influenza_scatter.png')
    ...
    #____processing_____
    df_location_data = sort_csv_data('data/wastewater_virus_surveillance/west/rvp/grouped_virus_avg_west_by_week.csv').tail(6).copy()
    ...
    df_asuW01 = sort_df_data(df_location_site_date[df_location_site_date['Site'].str.startswith('ASUW01')].drop(columns=['Site'])).sort_values(by='Date')
    df_asuW02 = sort_df_data(df_location_site_date[df_location_site_date['Site'].str.startswith('ASUW02')].drop(columns=['Site'])).sort_values(by='Date')
    df_asuW03 = sort_df_data(df_location_site_date[df_location_site_date['Site'].str.startswith('ASUW03')].drop(columns=['Site'])).sort_values(by='Date')
    ...
    p_freyja_date = px.bar(df_melt1,
                            ...
                            color='Variant',
                            title='ASU West Wastewater SARS-CoV-2 Variants by Date',
    ...
    p_freyja_site = px.bar(df_melt2,
                           ...
                           color_discrete_sequence=colors2,
                           color='Variant',
                           title='ASU West Wastewater SARS-CoV-2 Variants by Location',
                           # template='plotly_dark'
                           )
    ...
    p_ww_stacked_bar_historical = go.Figure(
        data=px.bar(df_ww_stacked_bar,
                    ...
                    title='Historically Ciruculating Viruses in ASU West Wastewater',
                    labels={
    ...
    p_ww_stacked_bar_current = px.bar(df_location_data1,
                                      ...
                                      title='Circulating Viruses in ASU West Wastewater',
                                      # template='plotly_dark'
    )
    ...
    #____West_location_abundance_by_date_graphing_____
    ...
    p1 = px.bar(df_asuW01,
                color_discrete_sequence=colors2,
                title='ASU West W1 (Las Casas)'
                )
    p2 = px.bar(df_asuW02,
                color_discrete_sequence=colors2,
                title='ASU West W2 (Casa De Oro)',
                )
    p3 = px.bar(df_asuW03,
                color_discrete_sequence=colors2,
                title='ASU West W3 (Verde Dining Pavilion)',
                )
    ...
    #____map____
    latitudes_list = [33.607793, 33.607597, 33.608389]

    longitudes_list = [-112.164718, -112.162418, -112.160879]
    ...
    m = folium.Map(location=[33.607980, -112.162960], zoom_start=17)
    ...
    st.markdown(f'#### Last Updated: Mar-22-2023')
    st.markdown('---')
    st.write('''
        ### West Collection Locations
        The map and table below displays West campus sample collection locations and their corresponding IDs, and facilities.
    ...
    with col_a3:
        tab1, tab2, tab3 = st.tabs(['W1', 'W2', 'W3'])
        with tab1:
            st.plotly_chart(p1)
        with tab2:
            st.plotly_chart(p2)
    ...
    st.markdown('---')
    st.markdown('### West Wastewater SARS-CoV-2 Freyja Analysis')
    ...
    st.markdown('---')
    st.markdown('### West wastewater RVP Analysis')
