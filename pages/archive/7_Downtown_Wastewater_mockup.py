...
if check_password():
    ...
    @st.cache_data
    def location():
        df = pd.read_csv('data/wastewater_virus_surveillance/downtown/downtown_locations.csv')
        df = df.set_index('ID')
        return df
    ...
    def read_distribution():
        df = pd.read_csv('data/wastewater_virus_surveillance/downtown/downtown_read_distribution_table.csv')
    ...
    @st.cache_data
    def freyja_by_date():
        df = pd.read_csv('data/wastewater_virus_surveillance/downtown/freyja/freyja_grouped_avg_by_date_dt.csv')
        return df


    @st.cache_data
    def freyja_by_site():
        df = pd.read_csv('data/wastewater_virus_surveillance/downtown/freyja/freyja_grouped_avg_by_site_dt.csv')
    ...
    adeno_current = Image.open('data/wastewater_virus_surveillance/images/current/downtown/scatter/weekly_counts_Adenovirus_scatter.png')
    cov2_historical = Image.open('data/wastewater_virus_surveillance/images/historical/grouped_counts_SARS-CoV-2_scatter.png')
    cov2_current = Image.open('data/wastewater_virus_surveillance/images/current/downtown/scatter/weekly_counts_SARS-CoV-2_scatter.png')
    flu_historial = Image.open('data/wastewater_virus_surveillance/images/historical/grouped_counts_Influenza_scatter.png')
    flu_current = Image.open('data/wastewater_virus_surveillance/images/current/downtown/scatter/weekly_counts_Influenza_scatter.png')
    ...
    #____processing_____
    df_location_data = sort_csv_data('data/wastewater_virus_surveillance/downtown/rvp/grouped_virus_avg_dt_by_week.csv').tail(6).copy()
    ...
    df_asuDT01 = sort_df_data(df_location_site_date[df_location_site_date['Site'].str.startswith('ASUDT01')].drop(columns=['Site'])).sort_values(by='Date')
    df_asuDT02 = sort_df_data(df_location_site_date[df_location_site_date['Site'].str.startswith('ASUDT02')].drop(columns=['Site'])).sort_values(by='Date')
    ...
    p_freyja_date = px.bar(df_melt1,
                            ...
                            color='Variant',
                            title='ASU Downtown Wastewater SARS-CoV-2 Variants by Date',
    ...
    p_freyja_site = px.bar(df_melt2,
                           ...
                           color_discrete_sequence=colors2,
                           color='Variant',
                           title='ASU Downtown Wastewater SARS-CoV-2 Variants by Location',
                           # template='plotly_dark'
                           )
    ...
    p_ww_stacked_bar_historical = go.Figure(
        data=px.bar(df_ww_stacked_bar,
                    ...
                    title='Historically Ciruculating Viruses in ASU Downtown Wastewater',
                    labels={
    ...
    p_ww_stacked_bar_current = px.bar(df_location_data1,
                                      ...
                                      title='Circulating Viruses in ASU Downtown Wastewater',
                                      # template='plotly_dark'
    )
    ...
    #____Downtown_location_abundance_by_date_graphing_____
    ...
    p1 = px.bar(df_asuDT01,
                color_discrete_sequence=colors2,
                title='ASU Downtown P1 (University Center)'
                )
    p2 = px.bar(df_asuDT02,
                color_discrete_sequence=colors2,
                title='ASU Downtown P2 (Law)',
                )
    ...
    #____map____
    latitudes_list = [33.453542, 33.453527]
    ...
    m = folium.Map(location=[33.453498, -112.072538], zoom_start=18)
    ...
    st.markdown('---')
    st.write('''
        ### Downtown Collection Locations
        The map and table below displays Downtown campus sample collection locations and their corresponding IDs, and facilities.
    ...
    st.markdown('---')
    st.markdown('### Downtown Wastewater SARS-CoV-2 Freyja Analysis')
    ...
    st.markdown('---')
    st.markdown('### Downtown wastewater RVP Analysis')
